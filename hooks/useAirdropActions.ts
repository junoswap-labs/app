...
export function useCreateAirdropCampaign() {
    ...
    const mutation = useMutation({
        mutationFn: async (input: CreateAirdropCampaignInput) => {
            ...
            if (!res.ok) {
                ...
                throw new Error(body?.error ?? `saving campaign details failed: ${res.status}`)
            }
    ...
    return { ...mutation, createCampaignAsync: mutation.mutateAsync }
}
...
export function useClaimAirdrop() {
    ...
    return { ...mutation, claimAsync: mutation.mutateAsync }
}
