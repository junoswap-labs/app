    ...
    return (
        <ol className="flex flex-nowrap items-center gap-1 overflow-x-auto">
            {steps.map((step, i) => {
                ...
                return (
                    <li key={step.status} className="flex shrink-0 items-center gap-1">
                        <span className="flex shrink-0 items-center gap-1">
                            ...
                            <span className={cn('whitespace-nowrap text-xs', current ? 'font-medium text-foreground' : 'text-muted-foreground')}>
                                {step.label}
                            </span>
                        </span>
                        {i < steps.length - 1 && (
                            <span className={cn('h-px w-3 shrink-0 sm:w-6', i < activeIndex ? 'bg-primary' : 'bg-border')} aria-hidden />
                        )}
                    </li>
                )
            })}
